...
rf <- setHyperPars(rf, importance="impurity")

rf.fit <- train(rf, dataset_tr)
saveRDS(rf.fit, file.path(path,"03_results/rf/rf_HCV_counts_freq_1_50.rds"))
#rf.fit <- readRDS(file.path(path,"03_results/rf/rf_HCV_counts_freq_1_50.rds"))
```

Apply model to the trainig data
```{r}
pred_tr <- predict(rf.fit, dataset_tr)
write.csv(as.data.frame(pred_tr), file.path(path,"03_results/rf/predictions_tr_rf_HCV_counts_freq_1_50.csv"))
```
...
write.csv(as.data.frame(pred_ts), file.path(path,"03_results/rf/predictions_ts_rf_HCV_counts_freq_1_50.csv"))
pr_grpl <- perf_binned_perf_curve(pred_ts, 20,"rec", "prec")
plot(pr_grpl$curve$rec_binned, pr_grpl$curve$prec)
write.csv(pr_grpl$curve, file.path(path,"03_results/rf/PR_rf_HCV_counts_freq_1_50.rds"))
```
...
ts_matches <- as.factor(data_ts$test_patient_id)
ids_ts <- utils_get_ids(data_ts, var_config)
data_ts <- utils_get_variables(data_ts, var_config, categorical = TRUE)
data_ts$PAT_GENDER_CD = as.numeric(data_ts$PAT_GENDER_CD) - 1
data_ts$label <- as.factor(data_ts$label)
data_ts[is.na(data_ts)] <- 0
...
```{r}
pred_ts <- predict(rf.fit, dataset_ts)
write.csv(as.data.frame(pred_ts), file.path(path,"03_results/rf/predictions_ts_rf_HCV_counts_freq_1_50_2.csv"))
pr_grpl <- perf_binned_perf_curve(pred_ts, 20,"rec", "prec")
plot(pr_grpl$curve$rec_binned, pr_grpl$curve$prec)
write.csv(pr_grpl$curve, file.path(path,"03_results/rf/PR_rf_HCV_counts_freq_1_50_2.rds"))
```
