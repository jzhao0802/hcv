...
data_tr <- readRDS(file.path(path, "/01_data/ensemble_lr_rf_xgb_tr.rds"))
...
saveRDS(xgb.fit , file.path(path,"03_results/ensemble/xgb_HCV_1_50.rds"))
```

Apply the model to the test data. 
```{r}
pred_tr <- predict(xgb.fit, dataset_tr)
write.csv(as.data.frame(pred_tr), file.path(path,"03_results/ensemble/predictions_tr_xgb_HCV_1_50.csv"))
```
...
write.csv(as.data.frame(pred_ts), file.path(path,"03_results/ensemble/predictions_ts_xgb_HCV_1_50.csv"))
pr_grpl <- perf_binned_perf_curve(pred_ts, 20,"rec", "prec")
plot(pr_grpl$curve$rec_binned, pr_grpl$curve$prec)
write.csv(pr_grpl$curve, file.path(path,"03_results/ensemble/PR_xgb_HCV_1_50.csv"))
```
