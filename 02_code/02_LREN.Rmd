...
```{r}
lren <- makeLearner("classif.glmnet", predict.type = "prob", predict.threshold = 0.5)
lren.fit <- train(lren, dataset_tr)
max_lambda <- max(lren.fit$learner.model$lambda)
saveRDS(lren.fit, file.path(path,"03_results/LR/LR_HCV_flags_1_50.rds"))
```
